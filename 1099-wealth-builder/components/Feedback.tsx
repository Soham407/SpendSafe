      ...
      <div className="bg-white/95 backdrop-blur-xl border border-indigo-100 p-6 rounded-[2.5rem] shadow-2xl text-center">
        {submitted ? (
          <div className="space-y-3 animate-in fade-in duration-500">
            ...
            <p className="text-xs font-black text-emerald-600 uppercase tracking-widest">Thanks for the input!</p>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Was this easy?</h4>
            ...
            <div className="flex justify-center gap-4">
              {[
                { icon: <Heart className="w-6 h-6 text-red-500" />, label: 'Love' },
                { icon: <Smile className="w-6 h-6 text-emerald-500" />, label: 'Easy' },
                { icon: <Meh className="w-6 h-6 text-gray-400" />, label: 'Okay' },
                { icon: <Frown className="w-6 h-6 text-red-400" />, label: 'Hard' }
              ].map((item) => (
                <button 
                  key={item.label}
            ...
            <p className="text-[9px] text-gray-300 font-bold italic">Building the Bootstrapper Edition v1.0</p>
          </div>
        )}
      </div>
    </div>
