...
import { ShieldCheck, Eye, Lock, ArrowRight, Building2, Calculator, TrendingUp, ChevronRight, Landmark, Loader2, X } from 'lucide-react';

interface OnboardingProps {
  onConnect: () => void;
}

export const Onboarding: React.FC<OnboardingProps> = ({ onConnect }) => {
  ...
  if (showPlaid) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col p-6 animate-in slide-in-from-bottom duration-500">
          ...
          <div className="grid grid-cols-1 gap-4">
            {['Chase', 'Wells Fargo', 'Bank of America', 'Capital One', 'Mercury', 'Found'].map((bank) => (
              <button
                ...
                className="w-full p-6 bg-white border border-gray-100 rounded-3xl flex items-center justify-between hover:border-indigo-600 transition-all group shadow-sm"
              >
        ...
        {isConnecting && (
          <div className="fixed inset-0 bg-white/90 backdrop-blur-md flex flex-col items-center justify-center z-50 p-12 text-center">
            ...
            <p className="text-gray-400 text-sm font-medium mt-2">Linking 1099 Wealth Builder to your business account.</p>
          </div>
        )}
      </div>
    );
