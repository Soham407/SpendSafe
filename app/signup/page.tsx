  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (signUpData.user) {
      ...
      const { error: profileError } = await supabase.from("profiles").insert({
        ...
        email: email,
        tax_rate_percentage: 0.3,
        retirement_rate_percentage: 0.1,
      });
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 animate-in">
      ...
      <p className="mt-8 text-xs text-muted text-center max-w-sm">
        SafeSpend provides educational estimates based on your inputs. This is
        not tax, legal, or financial advice. Consult a CPA for personalized
        guidance.
      </p>
    </div>
  );
